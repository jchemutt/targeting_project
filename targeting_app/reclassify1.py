...
def reclassify(input_raster_path, output_raster_path, reclassification_rules=None, allow_overwrite=False):
    ...
    with rasterio.open(input_raster_path) as src:
        data = src.read(1, masked=True).astype(np.float32)  # Ensure float32 for processing
        meta = src.meta.copy()

        # Preserve NoData values
        nodata = src.nodata if src.nodata is not None else NO_DATA_VALUE
        # data is already a MaskedArray when read with masked=True; ensure nodata is also masked
        ...
        for i, (min_val, max_val, new_value) in enumerate(reclassification_rules):
            is_last = (i == len(reclassification_rules) - 1)
            if is_last:
                mask = (masked_data >= min_val) & (masked_data <= max_val) & ~masked_data.mask
            else:
                mask = (masked_data >= min_val) & (masked_data < max_val) & ~masked_data.mask
